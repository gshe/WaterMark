...
- (NSURL *)applicationDocumentsDirectory {
  ...
  // code uses a directory named "com.gshe.WaterMark" in the user's Application
  // Support directory.
  NSURL *appSupportURL = [[[NSFileManager defaultManager]
      URLsForDirectory:NSApplicationSupportDirectory
             inDomains:NSUserDomainMask] lastObject];
  return [appSupportURL URLByAppendingPathComponent:@"com.gshe.WaterMark"];
}
...
- (NSManagedObjectModel *)managedObjectModel {
  ...
  NSURL *modelURL =
      [[NSBundle mainBundle] URLForResource:@"WaterMark" withExtension:@"momd"];
  _managedObjectModel =
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  // application to it. (The directory for the store is created, if necessary.)
  ...
  if (properties) {
    if (![properties[NSURLIsDirectoryKey] boolValue]) {
      failureReason =
          [NSString stringWithFormat:@"Expected a folder to store application "
                                     @"data, found a file (%@).",
